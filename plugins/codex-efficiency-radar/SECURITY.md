...
The optional Windows selector overlay is experimental and is not an official
OpenAI extension point. It launches the signed Microsoft Store Codex executable
with a Chrome DevTools Protocol endpoint bound to `127.0.0.1`, then injects UI
...
The overlay refuses to run unless the package version, app version, executable
version, and `app.asar` SHA-256 all match a reviewed entry in
...
Only install the overlay on a trusted local Windows account. Other local
processes running as the same user may be able to access the loopback debugging
endpoint while Codex is open. Use `Uninstall.cmd` or
`scripts/uninstall-selector-overlay.ps1` to stop the background process and
