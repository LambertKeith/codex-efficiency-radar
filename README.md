...
或 macOS Codex 模型选择器底部增加“查看效率值”入口。入口展开后以模型为行、
推理强度为列完整展示“综合智能”与“软件工程”两项数值。
...
- 原生选择器增强：兼容旧版菜单与新版 simple / advanced 界面，在任意选择器界面底部提供独立入口；
- 不再向模型或推理强度选项行内追加数值；矩阵面板底部提供刷新、失败提示与重试；
...
任意模型选择器，验证底部的“查看效率值”按钮及展开后的矩阵表格。
